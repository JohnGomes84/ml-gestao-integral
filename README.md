...
│  ├─ _core/                   # Bootstrap e infraestrutura base
│  ├─ db/                      # Conexão e acesso a dados
│  ├─ drizzle/                 # Schema e migrações
│  ├─ routes/                  # Definição principal de rotas/API
│  ├─ routers/                 # Routers organizados por domínio
│  ├─ modules/                 # Módulos de negócio (ex.: payroll)
...
Antes de abrir uma PR interna:

1. Execute `pnpm check` para validar tipagem.
2. Execute `pnpm test` para validar testes automatizados.
3. Atualize a documentação em `docs/` quando houver mudanças estruturais.
