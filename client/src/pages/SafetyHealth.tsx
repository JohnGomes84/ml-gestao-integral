...
export default function SafetyHealth() {
  const { data: ppeDeliveries, isLoading: ppeLoading } =
    trpc.ppeDeliveries.list.useQuery({});
  const { data: serviceOrders, isLoading: ordersLoading } =
    trpc.serviceOrders.list.useQuery({});
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Controle de EPIs, ordens de serviço, treinamentos e conformidade
            regulatória.
          </p>
        </div>
                      ...
                      {ppeDeliveries.map((ppe: any) => (
                        <TableRow key={ppe.id}>
                          <TableCell className="font-medium">
                            {ppe.employeeName || `ID ${ppe.employeeId}`}
                          </TableCell>
                          ...
                          <TableCell>
                            {formatDate(ppe.returnDate) || "Pendente"}
                          </TableCell>
                          <TableCell>
                            <Badge
                              variant={
                                ppe.employeeSignature ? "default" : "secondary"
                              }
                            >
                              {ppe.employeeSignature
                                ? "Assinado"
                                : "Não assinado"}
                            </Badge>
                          </TableCell>
                    ...
                    <TableBody>
                      {serviceOrders.map((order: any) => (
                        <TableRow key={order.id}>
                          <TableCell className="font-medium">
                            {order.employeeName || `ID ${order.employeeId}`}
                          </TableCell>
