...
import { eq, and, gte, lte, lt, desc, asc, sql, count, or, isNotNull, like } from "drizzle-orm";
...
export async function getDashboardStats() {
  ...
  const [totalResult, activeResult, statusCounts, overdueVacResult, expiredExamResult, unreadResult] = await Promise.all([
    ...
    db.select({ count: count() }).from(vacations)
      .where(and(sql`${vacations.concessionLimit} <= ${today}`, eq(vacations.status, "Pendente"))),
    db.select({ count: count() }).from(medicalExams)
      .where(and(sql`${medicalExams.expiryDate} <= ${today}`, eq(medicalExams.status, "Válido"))),
