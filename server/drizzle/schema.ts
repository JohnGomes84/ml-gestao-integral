...
export const users = mysqlTable("users", {
  ...
  role: mysqlEnum("role", ["admin", "manager", "user", "viewer"]).default(
    "user"
  ),
...
export const workers = mysqlTable(
  "workers",
  {
    ...
    pixKeyType: mysqlEnum("pix_key_type", [
      "cpf",
      "cnpj",
      "email",
      "phone",
    ...
    status: mysqlEnum("status", ["active", "inactive", "blocked"]).default(
      "inactive"
    ),
...
export const workerRefusals = mysqlTable("worker_refusals", {
  ...
  refusalType: mysqlEnum("refusal_type", [
    "scheduling_conflict",
    "distance",
    "rate_too_low",
    "personal_reasons",
...
export type InsertWorkerAutonomyMetrics =
  typeof workerAutonomyMetrics.$inferInsert;
...
export const payments = mysqlTable("payments", {
  ...
  status: mysqlEnum("status", ["pending", "paid", "cancelled"]).default(
    "pending"
  ),
...
export const workOffers = mysqlTable("work_offers", {
  ...
  jobFunction: varchar("job_function", { length: 255 }),
  dailyRate: decimal("daily_rate", { precision: 10, scale: 2 }),
  response: mysqlEnum("response", ["pending", "accepted", "refused"]).default(
    "pending"
  ),
...
export const epiRecords = mysqlTable("epi_records", {
  ...
  status: mysqlEnum("status", ["active", "expired", "returned"]).default(
    "active"
  ),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow(),
...
export const timeBank = mysqlTable("time_bank", {
  ...
  expiryDate: datetime("expiry_date"),
  status: mysqlEnum("status", ["Ativo", "Expirado", "Compensado"]).default(
    "Ativo"
  ),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow(),
...
export const benefits = mysqlTable("benefits", {
  ...
  startDate: datetime("start_date").notNull(),
  endDate: datetime("end_date"),
  status: mysqlEnum("status", ["Ativo", "Cancelado", "Suspenso"]).default(
    "Ativo"
  ),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow().onUpdateNow(),
...
export const equipmentLoans = mysqlTable("equipment_loans", {
  ...
  status: mysqlEnum("status", ["Ativo", "Devolvido", "Atrasado"]).default(
    "Ativo"
  ),
  observations: text("observations"),
  createdAt: timestamp("created_at").defaultNow(),
