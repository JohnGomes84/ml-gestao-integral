...
function isValidPhone(phone: string): boolean {
  ...
  // Telefone brasileiro: +55 (DDD) 9XXXX-XXXX ou +55 (DDD) XXXX-XXXX
  // Formato: 5511999999999 (13 dígitos) ou 551199999999 (12 dígitos para fixo)
  if (cleanPhone.startsWith('55')) {
    return cleanPhone.length === 13 || cleanPhone.length === 12;
  }
  
  // Formato sem código do país: (DDD) 9XXXX-XXXX
...
function isValidRandomKey(key: string): boolean {
  ...
  const randomKeyRegex = /^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/i;
...
export function validatePixKey(key: string): PixValidationResult {
  ...
  if ((cleanKey.length === 10 || cleanKey.length === 12 || cleanKey.length === 13) && isValidPhone(key)) {
    return {
      isValid: true,
      type: 'phone'
    };
  ...
  return {
    isValid: false,
    type: null,
    error: 'Chave PIX inválida. Use CPF, CNPJ, Email, Telefone ou Chave Aleatória válidos.'
  };
}
