...
describe("Risk Dashboard Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const worker1Result = await db.insert(workers).values({
    ...
    testWorkerId1 = Number(
      (Array.isArray(worker1Result) ? worker1Result[0] : worker1Result).insertId
    );

    const worker2Result = await db.insert(workers).values({
    ...
    testWorkerId2 = Number(
      (Array.isArray(worker2Result) ? worker2Result[0] : worker2Result).insertId
    );

    const worker3Result = await db.insert(workers).values({
    ...
    testWorkerId3 = Number(
      (Array.isArray(worker3Result) ? worker3Result[0] : worker3Result).insertId
    );
    ...
    for (let i = 4; i >= 0; i--) {
      ...
      const operationId = Number(
        (Array.isArray(operationResult) ? operationResult[0] : operationResult)
          .insertId
      );
    ...
    const operation3Result = await db.insert(operations).values({
      clientId: testClientId,
      locationId: testLocationId,
      operationName: "Test Operation",
      workDate: today,
    ...
    const operation3Id = Number(
      (Array.isArray(operation3Result) ? operation3Result[0] : operation3Result)
        .insertId
    );
  ...
  it("should calculate consecutive days correctly", async () => {
    ...
    const worker1Days = await calculateConsecutiveDays(
      testWorkerId1,
      testClientId
    );
    const worker2Days = await calculateConsecutiveDays(
      testWorkerId2,
      testClientId
    );
    const worker3Days = await calculateConsecutiveDays(
      testWorkerId3,
      testClientId
    );
  ...
  it("should handle workers with no operations", async () => {
    ...
    const timestamp = Date.now();
    const newWorkerResult = await db.insert(workers).values({
    ...
    const newWorkerId = Number(
      (Array.isArray(newWorkerResult) ? newWorkerResult[0] : newWorkerResult)
        .insertId
    );
